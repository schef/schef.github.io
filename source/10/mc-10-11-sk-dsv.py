...
print ("Decending melodic sevens pitch indetification drill. Were not singing the tone. We only identify them.")
...
try:
  print(usage)
  while True:
    noteOne = random.choice(notes[6:])
    noteTwo = notes[notes.index(noteOne)-6]
    #while True:
    match = False
    while not match:
      done = False
